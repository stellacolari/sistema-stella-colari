...
export function perfilNotificacaoUsuario(usuario: {
  perfil: string;
  perfilAdministrativo?: { codigo: string; ativo: boolean } | null;
}) {
  if (usuario.perfilAdministrativo?.ativo)
    return usuario.perfilAdministrativo.codigo;
...
function filtroAcesso(
  perfil: PerfilAdmin,
): Prisma.NotificacaoSistemaWhereInput {
...
async function garantirDestinatarios(
...
async function resolverDestinatarios(params: {
  ...
  const regrasEspecificas = regras.filter(
    (regra) => regra.tipoNotificacao === params.tipo,
  );
  const regrasAplicaveis = regrasEspecificas.length
    ? regrasEspecificas
    : regras;

  for (const regra of regrasAplicaveis) {
    if (!prioridadeAtendeMinima(params.prioridade, regra.prioridadeMinima))
      continue;
  ...
  if (perfis.size === 0 && usuarios.size === 0) {
    for (const perfil of params.perfisDestino?.length
      ? params.perfisDestino
      : destinoPadrao(params.categoria)) {
      perfis.add(
        mapearPerfilLegado(perfil) === "ADMIN_GERAL"
          ? "ACESSO_GERAL"
          : "VENDEDOR",
      );
      perfis.add(perfil);
...
async function garantirDestinatariosRegras(
  notificacaoId: string,
...
export async function criarOuAtualizarNotificacao(
  params: CriarNotificacaoParams,
) {
...
export async function listarNotificacoes(params: ListarParams) {
  ...
  if (params.prioridade && params.prioridade !== "TODAS")
  ...
  return prisma.notificacaoSistema
    .findMany({
      where,
      include: {
        destinatarios: {
    ...
    .then((notificacoes) =>
      notificacoes
        .map((notificacao) => {
          const destinatarioUsuario = notificacao.destinatarios.find(
            (item) => item.usuarioId === params.usuarioId,
          );
          const destinatarioPerfil = notificacao.destinatarios.find(
            (item) => item.perfilDestino === params.perfil,
          );
        ...
        .filter((notificacao) => {
          ...
          if (params.status && params.status !== "TODAS")
            return notificacao.status === params.status;
          return true;
        }),
    );
}
...
export async function contarNotificacoesNaoLidas(
  ...
  const notificacoes = (
    await listarNotificacoes({ usuarioId, perfil, take: 1000 })
  ).filter((item) => item.status === "NOVA");
  const total = notificacoes.length;
  const pedidos = notificacoes.filter(
    (item) => item.categoria === "PEDIDO",
  ).length;
  const reposicao = notificacoes.filter((item) =>
    ["ESTOQUE", "REPOSICAO"].includes(item.categoria),
  ).length;
  const recomendacoes = notificacoes.filter(
    (item) => item.categoria === "RECOMENDACAO",
  ).length;
  const campanhas = notificacoes.filter(
    (item) => item.categoria === "CAMPANHA",
  ).length;
  const precificacao = notificacoes.filter(
    (item) => item.categoria === "PRECIFICACAO",
  ).length;
...
async function destinatarioDoUsuario(
  ...
  if (!notificacao) {
    throw new Error(
      "Notificacao nao encontrada ou sem permissao para este perfil.",
    );
  }
...
export async function excluirVariasNotificacoes(
  ids: string[],
  usuarioId: string,
  perfil: PerfilAdmin,
) {
...
async function sincronizarPedidos() {
  ...
  for (const pedido of pedidos) {
    ...
    if (
      ...
      ["PEDIDO_RECEBIDO", "EM_SEPARACAO"].includes(pedido.status)
    ) {
      ativas.add(
        chaveNotificacao({
    ...
    if (
      ...
      await criarOuAtualizarNotificacao({
        ...
        descricao:
          "Pagamento pendente ha mais de 24 horas. Conferir link ou contato antes de cancelar.",
    ...
    if (["PROBLEMA", "PROBLEMA_OPERACIONAL"].includes(pedido.status)) {
      ...
      await criarOuAtualizarNotificacao({
        ...
        descricao:
          "Pedido marcado com problema operacional. Conferir antes de seguir.",
    ...
    if (pedido.status === "CANCELADO") {
      ...
      await criarOuAtualizarNotificacao({
        ...
        descricao:
          "Pedido cancelado deve ser conferido para garantir que pagamento, estoque e atendimento estejam coerentes.",
    ...
    if (
      ...
      await criarOuAtualizarNotificacao({
        ...
        descricao:
          "Pedido pago ainda precisa de etiqueta ou declaracao para envio.",
    ...
    if (
      ...
      await criarOuAtualizarNotificacao({
        ...
        descricao:
          "Pedido pago com retirada local ainda precisa ser entregue ao cliente.",
...
async function sincronizarRecomendacoes() {
  ...
  for (const recomendacao of recomendacoes) {
    if (recomendacao.prioridade !== "ALTA" && recomendacao.tipo !== "REPOSICAO")
      continue;
    const linkAcao =
      recomendacao.tipo === "REPOSICAO"
        ? "/compras/reposicao"
        : "/compras/recomendacoes";
    ativas.add(
      chaveNotificacao({
        tipo:
          recomendacao.tipo === "REPOSICAO"
    ...
    await criarOuAtualizarNotificacao({
      tipo:
        recomendacao.tipo === "REPOSICAO"
          ...
          : "RECOMENDACAO_GERENCIAL_NOVA",
      categoria:
        recomendacao.tipo === "REPOSICAO" ? "REPOSICAO" : "RECOMENDACAO",
  ...
  for (const recomendacao of vencidas) {
    ...
    await criarOuAtualizarNotificacao({
      ...
      descricao:
        "A recomendacao aceita ou em execucao passou do prazo sugerido e precisa de revisao.",
...
async function sincronizarPrecificacao() {
  ...
  for (const produto of criticos.slice(0, 30)) {
    ...
    await criarOuAtualizarNotificacao({
      ...
      prioridade:
        produto.classificacao === "PRECO_CRITICO" || produto.custoAusente
          ? "ALTA"
          : "MEDIA",
...
async function arquivarNotificacoesObsoletas(ativas: Set<string>) {
  ...
  const obsoletas = abertas.filter(
    (item) => !ativas.has(chaveNotificacao(item)),
  );
